...
export async function shallowCopy(user: User,
    ...
    if (sessionInfo) {
        if (sessionInfo.private === false) {
            ...
            await Promise.all(sessionSets.map(async (sessionGeneset) => await addToSessionSetsGeneObj(sessionGeneset.genes, newSession.id, sessionGeneset.name, sessionGeneset.description ? sessionGeneset.description : '', user, [], true)))
