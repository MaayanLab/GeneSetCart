...
export const Scatterplot = ({ width, height, data }: ScatterplotProps) => {
  ...
  const yScale = d3.scaleLinear().domain([Math.min(...ys) - 5, Math.max(...ys) + 5]).range([boundsHeight, 0]);
  const xScale = d3
    .scaleLinear()
    .domain([Math.min(...data.map((point) => point.x)) - 5, Math.max(...data.map((point) => point.x)) + 5])
    .range([0, boundsWidth]);
